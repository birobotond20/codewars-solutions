 ...
 * vertically; that means that if I'm in the leftmost and I try to go left again I'll get to the
 * rightmost (examples: from Ryu to Vega, from Ken to M.Bison) and vice versa from rightmost to
 * leftmost.
 *
 ...
 * Zangief, Dhalsim, Sagat and M.Bison in the above image; you can't go upper than highest row:
 ...
 *
 * as the characters I've been hovering with the selection cursor during my moves. Notice: Ryu
 * is the first just because it "fails" the first up See test cases for more examples.
 *
 * 2.
 *
 * fighters = [
...
public class Solution{
  ...
  public static void main(String[] args) {
    ...
    System.out.println(Arrays.toString(streetFighterSelection(fighters, new int[]{0,0}, new String[]{})));
  }

  public static String[] streetFighterSelection(String[][] fighters, int[] position, String[] moves){
    String[] hoveredOverFighters = new String[moves.length];
    int currentHorizontalIndex = position[0];
    int currentVerticalIndex = position[1];
    int maxHorizontalIndex = fighters[0].length - 1;
    int maxVerticalIndex = fighters.length - 1;

    if (moves.length == 0) {
      return hoveredOverFighters;
    } else {
      hoveredOverFighters[0] = fighters[currentHorizontalIndex] [currentVerticalIndex];
      for (int i = 0; i < moves.length; i++) {
        if (moves[i].equals("up")) {
          if (currentVerticalIndex - 1 >= 0) {
            currentVerticalIndex--;
          }
        } else if (moves[i].equals("down")) {
          if (currentVerticalIndex + 1 <= maxVerticalIndex) {
            currentVerticalIndex++;
          }
        } else if (moves[i].equals("left")) {
          if (currentHorizontalIndex - 1 < 0) {
            currentHorizontalIndex = maxHorizontalIndex;
          } else {
            currentHorizontalIndex--;
          }
        } else if (moves[i].equals("right")){
          if (currentHorizontalIndex + 1 > maxHorizontalIndex) {
            currentHorizontalIndex = 0;
          } else {
            currentHorizontalIndex++;
          }
        }
        hoveredOverFighters[i] = fighters[currentVerticalIndex][currentHorizontalIndex];
      }
    }
    return hoveredOverFighters;
  }
